Feature: Testing GameTime player API
  ...
  Scenario Outline: read player returns expectedhttp status
    When readplayer is called with "<id>"
    Then readplayer api returns http status code "<statuscode>"
    Examples:
      | id                                    | statuscode    |
      | 330eb324-3382-4fca-b20e-2b4c7e278047  | 200 OK        |
      | aaa                                   | 404 NOT_FOUND |
